...
async function getTasks() {
  ...
  if (rawTextBuffer && rawTextBuffer.length) {
    ...
    const allNotDone = Array.from(rawText.matchAll(notDoneRegex));
    ...
    allNotDone.forEach((value, index) => {
      const text = value[4];
      const line = lines.reduce((prev, curr, idx) => curr.includes(text) ? idx + 1 : prev)
      const newHeading = lines.slice(0, line - 1).reduceRight((prev, curr, idx) => {
        if (prev === "") {
          return curr.includes(':') ? curr : prev;
        }
        return prev;
      }, "")
